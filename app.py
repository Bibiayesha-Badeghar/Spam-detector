...
def load_model_and_vectorizer():
    ...
    try:
        ...
        return (
            None,
            None,
            f"Error: Model files not found. Please run train_model.py first. ({str(e)})",
        )
...
def save_feedback(email_text, actual_label, predicted_label, confidence):
    ...
    if os.path.exists(FEEDBACK_FILE):
        try:
            ...
            logger.warning(
                f"Could not read feedback file {FEEDBACK_FILE}. Starting fresh."
            )
            feedback_data = []
...
def feedback():
    ...
    try:
        ...
        # Save feedback
        success, result = save_feedback(
            email_text, actual_label, predicted_label, confidence
        )
        ...
        if success:
            ...
            return (
                jsonify(
                    {"success": False, "error": f"Could not save feedback: {result}"}
                ),
                500,
            )
        ...
        return (
            jsonify(
                {"success": False, "error": f"Error processing feedback: {str(e)}"}
            ),
            500,
        )
...
def status():
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "error": f"Failed to read feedback status: {e}"}
            ),
            500,
        )
    except Exception as e:
        logger.exception("Failed to retrieve status.")
        return jsonify({"success": False, "model_loaded": False, "error": str(e)}), 500
...
if __name__ == "__main__":
    ...
    # Open browser automatically when script runs (avoids duplicate tabs if reloader restarts)
